...
public class SHA256Gadget extends Gadget {
	...
	public SHA256Gadget(Wire[] ins, int bitWidthPerInputElement, int totalLengthInBytes, boolean binaryOutput,
		...
		if (totalLengthInBytes * 8 > ins.length * bitWidthPerInputElement
			...
			System.out.println(totalLengthInBytes * 8 + " > " + ins.length * bitWidthPerInputElement);
			System.out.println(totalLengthInBytes * 8 + " > " +(ins.length - 1) * bitWidthPerInputElement);
	...
	public Wire getOutputWiresNoArray() {
		Wire[] Bits = new Wire[32];
		Wire[] result = new Wire[256];

		for (int i = 0; i < 8; i++) {
			Bits = output[i].getBitWires(32).asArray();
			for (int j = 0; j < 32; j++)
				result[32 * i + j] = Bits[j];
		}
		Wire out = new WireArray(result).packAsBits();
		return out;
	}
}
