...
#include <libsnark/zk_proof_systems/ppzksnark/snark_friendly_enc/r1cs_gg_ppzksnark.hpp>
#include <libsnark/zk_proof_systems/ppzksnark/snark_friendly_enc/SNARK_friendly.hpp>
                        ...
                        const bool test_serialization)
{
    ...
    std::string m = "F0FFFFFFFFFFFFFFFFFFFFFFFFFFFF0F";
    libff::enter_block("SF Encrypt");
    SF_cypher_text<ppT> SF_ct = SF_encrypt(keypair, SF_key.pk, m,example.auxiliary_input); // Create Enc Proof
    libff::leave_block("SF Encrypt");
	// Flag , File Ouput , 
    libff::print_header("SNARK Friendly Encrypt Verifier");
    libff::enter_block("SF Enc Verifier");
    bool ans = SF_enc_verifier(keypair, SF_key.vk, SF_ct.proof, SF_ct);//Encryption Proof
    printf("* The verification :result is: %s\n", (ans ? "PASS" : "FAIL"));
    libff::leave_block("SF Enc Verifier");
    
    libff::print_header("SNARK Friendly Decryption");
    libff::enter_block("SF Decrypt");
    SF_plain_text<ppT> SF_pt = SF_decrypt(keypair,SF_key.sk,SF_ct,example.auxiliary_input); // Create Dec proof
    ...
    bool ans2 = SF_dec_verifier(keypair, SF_key.vk, SF_ct.proof,SF_pt, SF_ct); // Decryption Proof
    printf("* The verification result is: %s\n", (ans2 ? "PASS" : "FAIL"));
