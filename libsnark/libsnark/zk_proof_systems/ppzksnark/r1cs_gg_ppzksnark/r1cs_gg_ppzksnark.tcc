...
template<typename ppT>
bool r1cs_gg_ppzksnark_getGenerator<ppT>::operator==(const r1cs_gg_ppzksnark_getGenerator<ppT> &other) const
{
    return (this->G1 == other.G1 &&
            this->G2 == other.G2 
            );
}
...
{
    out << G.G1 << OUTPUT_NEWLINE;
    out << G.G2 << OUTPUT_NEWLINE;
...
r1cs_gg_ppzksnark_keypair<ppT> r1cs_gg_ppzksnark_generator(const r1cs_gg_ppzksnark_constraint_system<ppT> &r1cs)
{
    ...
    libff::G1<ppT> g1_generator = libff::G1<ppT>::random_element();
    ...
    //G = r1cs_gg_ppzksnark_getGenerator<ppT>(libff::Fr<ppT>::one() * g1_generator, libff::Fr<ppT>::one() * G2_gen);
