...
template<typename ppT>
class r1cs_gg_ppzksnark_getGenerator;

template<typename ppT>
std::ostream& operator<<(std::ostream &out, const r1cs_gg_ppzksnark_getGenerator<ppT> &G);

template<typename ppT>
std::istream& operator>>(std::istream &in, r1cs_gg_ppzksnark_getGenerator<ppT> &G);
...
public:
    ...
    r1cs_gg_ppzksnark_getGenerator() {};
    r1cs_gg_ppzksnark_getGenerator<ppT>& operator=(const r1cs_gg_ppzksnark_getGenerator<ppT> &other) = default;
    r1cs_gg_ppzksnark_getGenerator(const r1cs_gg_ppzksnark_getGenerator<ppT> &other) = default;
    r1cs_gg_ppzksnark_getGenerator(r1cs_gg_ppzksnark_getGenerator<ppT> &&other) = default;
    r1cs_gg_ppzksnark_getGenerator(libff::G1<ppT> &&G1,
                                  libff::G2<ppT> &&G2) :
        G1(G1),
        G2(G2)
    {};
    ...
    size_t G1_size() const
    {
        return 1 * libff::G1<ppT>::size();
    }

    size_t G2_size() const
    {
        return 1 * libff::G2<ppT>::size();
    }


    size_t size_in_bits() const
    {
        return (1 * libff::G1<ppT>::size_in_bits() + 1 * libff::G2<ppT>::size_in_bits());
    }

    void print_size() const
    {
        libff::print_indent(); printf("* G1 elements : %zu\n", this->G1_size());
        libff::print_indent(); printf("* G2 elements : %zu\n", this->G2_size());
        libff::print_indent(); printf("* Generator size in bits: %zu\n", this->size_in_bits());
    }

    bool operator==(const r1cs_gg_ppzksnark_getGenerator<ppT> &other) const;
    friend std::ostream& operator<< <ppT>(std::ostream &out, const r1cs_gg_ppzksnark_getGenerator<ppT> &pk);
    friend std::istream& operator>> <ppT>(std::istream &in, r1cs_gg_ppzksnark_getGenerator<ppT> &pk);
};
