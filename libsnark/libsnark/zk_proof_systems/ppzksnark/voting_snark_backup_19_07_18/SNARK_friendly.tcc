...
SF_cypher_text<ppT> SF_encrypt( const r1cs_gg_ppzksnark_keypair<ppT> &gg_keypair, 
    ...
    for(size_t i = 196; i < r1cs_primary_input.size();i++){
...
SF_plain_text<ppT> SF_decrypt( const r1cs_gg_ppzksnark_keypair<ppT> &gg_keypair, 
    ...
    libff::Fr<ppT> r_new = libff::Fr<ppT>::zero();
    ...
    for (size_t ct_i = 0; ct_i < ct.size(); ct_i++){
        ...
        for (size_t i = 1; i < ct[ct_i].G1_ct.size() - 1; i++){
            libff::GT<ppT> ci_sk_0 = ppT::reduced_pairing(ct[ct_i].G1_ct[i], sk.H_pub[0]);
            libff::GT<ppT> c0_sk_i = ppT::reduced_pairing(ct[ct_i].G1_ct[0], sk.H_pub[i]);
        ...
        r_new = r_new + libff::Fr<ppT>(r_bndec);
        m_new = m_new + libff::Fr<ppT>(bg_msg);
    }

    std::cout << "m_new ::: "; m_new.print();
    std::cout << "r_new ::: "; r_new.print();
    libff::bigint<4> bg_m_new = m_new.as_bigint();
    char char_arr[33] = "";
    gmp_sprintf(char_arr,"%032NX", bg_m_new.data, bg_m_new.N);
    printf(":::::%s\n",char_arr);
    std::string str_m_new = std::string(char_arr);

    SF_plain_text<ppT> pt = SF_plain_text<ppT>(r_new,str_m_new);
...
bool SF_dec_verifier(const SF_public_key<ppT> &pk,
    ...
    libff::G1<ppT> acc_c = libff::G1<ppT>::zero();
    for(size_t i = 0; i < ct_vect.size(); i++)
        acc_c = acc_c + ct_vect[i].G1_ct[ct_vect[i].G1_ct.size()-1];

    libff::bigint<4> bg_msg = libff::bigint<4>(pt.msg.c_str(),16);
    libff::Fr<ppT> fr_msg = libff::Fr<ppT>(bg_msg);
    libff::G1<ppT> G1_ver = pt.r_new*pk.G_pub[193] + fr_msg*pk.G_pub[194];
